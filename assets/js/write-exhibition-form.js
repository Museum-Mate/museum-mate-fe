function postExhibitionInfo() {
	...
	form.addEventListener(
		...
		function (e) {
			...
			let exhibitionDetailInfo = document.getElementById(
				"exhibitionDetailInfo"
			).value;
			let exhibitionNotice = document.getElementById("exhibitionNotice").value;
			const mainImgUrl = document
				.getElementById("MainImgframe")
				.getAttribute("src");
			const detailInfoImgUrl = document
				.getElementById("DetailInfoImgframe")
				.getAttribute("src");
			const noticeImgUrl = document
				.getElementById("NoticeImgframe")
				.getAttribute("src");
			...
			fetch("http://localhost:8080/api/v1/exhibitions/new", {
				...
				.then(function (data) {
					...
					if(resultCode === 'SUCCESS') {
						alert("등록 완료");
						window.location.replace('http://localhost:3000/write-exhibition');
					}
				})
